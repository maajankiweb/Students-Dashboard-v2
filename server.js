...
if (!MONGODB_URI) {
  ...
  console.error('Set it in Render.com → Dashboard → your service → Environment → Add Environment Variable (key: MONGODB_URI, value: your MongoDB connection string).');
  process.exit(1);
}
...
async function appHandler(req, res) {
  try {
    ...
    if (req.method === 'POST') {
      ...
      switch (req.body.action) {
        ...
        case 'save_attendance': {
          ...
          await Attendance.deleteMany({ date: date, subject_id: subject_id, course_id: course_id });
          if (attendance) {
      ...
      return res.redirect(`/app?page=${page}&msg=${msg}`);
    }
    ...
    if (page === 'staff_notifs' || page === 'student_notifs') {
      ...
      data.notifs = await Notification.find({ type: type }).sort('-created_at');
    }
